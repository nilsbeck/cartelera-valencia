"""
Cines MN4 Valencia scraper.
URL: https://www.reservaentradas.com/cine/valencia/cinesmn4

Same platform and DOM as Cines Lys — see lys.py for full documentation.
...
CINEMA_PATH = "/cine/valencia/cinesmn4"
...
def _fetch_session_map(
    ...
    The page is JS-rendered: date tabs and /entrada/ booking links are
    injected by client-side JS after DOMContentLoaded.  Waiting for an
    /entrada/ href is the definitive signal that the session list has
    rendered.
    """
    status = None
    wait_ok = False
    try:
        ...
        if diag:
            print(f"  [mn4] DIAG sesiones goto failed for {sesiones_url}: {e}", flush=True)
        return {}
    ...
    if diag:
        try:
            ...
            print(
                f"  [mn4] DIAG sesiones (status={status} wait_ok={wait_ok}): {d}",
                flush=True,
            )
        except Exception as e:
            print(f"  [mn4] DIAG evaluate failed: {e}", flush=True)
...
def scrape() -> list[dict]:
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if not phase1:
                print("  ⚠ MN4 page loaded but found no movie blocks")
    ...
    if phase1 and phase2_hits == 0:
        print(f"  ⚠ MN4 Phase 2 returned no sessions for any of {len(phase1)} movies "
    ...
    for entry in phase1:
        ...
        if session_map:
            ...
            print(f"  ⚠ MN4 Phase 2 empty for '{entry['title']}', using today's sessions")
